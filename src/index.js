...
export default function useMediaCarousel(props) {
  ...
  const customAudioPlayer = props.customAudioPlayer
    ? props.customAudioPlayer
    : undefined;
  const disableSwipe = props.disableSwipe ? props.disableSwipe : undefined;
  const enableMarkDown = props.enableMarkDown;
  ...
  useEffect(() => {
    ...
    if (slideShow && open) {
      ...
      if (
        activeVideoPlayer.current &&
        activeVideoPlayer.current.currentTime === 0
      ) {
        activeVideoPlayer.current.play();
      } else if (
        activeAudioPlayer.current &&
        activeAudioPlayer.current.currentTime === 0
      ) {
        activeAudioPlayer.current.play();
      } else {
        if (activeSlide === slides.length - 1) {
  ...
  const close = React.useCallback(() => {
    console.log("Closing Media Carousel");
